...
List<Widget> examples(bool? hasPermission) => <Widget>[
  ...
  if (hasPermission ?? false)
    ...
    const Text('WARNING: The user did not grant permissions to show notifications'),
...
final Uri badgeUrl = Uri.parse(AssetManager().getAssetUrl('icons/app_icon_density.png'));

Future<void> showDetails(WebNotificationDetails details) => flutterLocalNotificationsPlugin
  .show(
    id++,
    'This is a title',
    'This is a body',
...
void _showTimestamp() => showDetails(
  WebNotificationDetails(timestamp: DateTime(2020, 1, 5))
);
...
void _showInteraction() => showDetails(
  const WebNotificationDetails(requireInteraction: true)
);
...
void _showRtl() => flutterLocalNotificationsPlugin
  .show(
    id++,
