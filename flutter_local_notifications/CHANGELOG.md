...
* Added the `zonedSchedule` method to the plugin that allows for scheduling notifications to occur on a specific date and time, within a specific time zone. The `schedule` method has been marked as a deprecated due to problems with time zones, particularly when it comes to daylight savings. Note that to support time zone-based scheduling, the plugin now depends on the `timezone` package so that an instance of the `TZDateTime` class is required to the specify the time the notification should occur. This should work in most cases as it is IANA-based and native platforms have time zones that are IANA-based as well. Note that on Android, the [ThreeTen Android Backport library](https://github.com/JakeWharton/ThreeTenABP) is used to supported time zone aware dates on older versions of Android
