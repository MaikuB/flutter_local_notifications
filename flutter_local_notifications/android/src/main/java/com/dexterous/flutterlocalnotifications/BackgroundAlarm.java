...
class BackgroundAlarm {
  ...
  static void start(final Context context, NotificationDetails notificationDetails) {
    if (notificationDetails.playSound) playAlarmSound(context, notificationDetails);
    if (notificationDetails.enableVibration) vibrate(context, notificationDetails);
  }
  ...
  private static void playAlarmSound(
      final Context context, NotificationDetails notificationDetails) {
    Ringtone r =
  ...
  private static void setRingtoneLength(Ringtone r, NotificationDetails notificationDetails) {
    ...
    r.setLooping(true);
    new Handler(Looper.getMainLooper()).postDelayed(r::stop, notificationDetails.timeoutAfter);
  }
  ...
  private static void vibrate(final Context context, NotificationDetails notificationDetails) {
    ...
    new Handler(Looper.getMainLooper()).postDelayed(v::cancel, notificationDetails.timeoutAfter);
  }
