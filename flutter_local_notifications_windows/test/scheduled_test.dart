...
void main() => group("Schedules", () {
  ...
  Future<int> countPending() async => (await plugin.pendingNotificationRequests()).length;
  late final location = getLocation("US/Eastern");
  ...
  test("do not work with earlier time", () async {
    ...
    await plugin.cancelAll();
    expect(await countPending(), 0);
    expect(plugin.zonedSchedule(302, null, null, now, null), throwsArgumentError);
    expect(plugin.zonedSchedule(302, null, null, earlier, null), throwsArgumentError);
  });
});
